...
def write_grayscale(filename, pixels):
    ...
    Args: 
        ...
        pixels: A rectangular image stored as a sequence of roes.
    ...
    with open(filename, 'wb') as bmp:
        ...
        #Image Header
        bmp.write(b'\x28\x00\x00\x00') # Image header size in bytes - 40 deecimal
        ...
        bmp.write(b'\x00\x00\x00\x00') # Unused pixels per meter
        bmp.write(b'\x00\x00\x00\x00') # Use whole coloe table
...
def dimensions(filename):
    '''Detemine the dimensions in pixels of a BMP image.
    ...
    with open(filename, 'rb') as f:
        # Check if the file is a BMP file by reading the first 2 bytes
        magic = f.read(2)
        if magic != b'BM':
